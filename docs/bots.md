...
| `newsSources` | string[] | Generated by LLM | News sources the bot monitors for hot topics. Three entry forms: (1) an RSS feed URL; (2) `Search: <query>`, which resolves to a Google News search RSS feed — each result is attributed to its real publisher (from the item's `<source>` element, falling back to the `" - Publisher"` title suffix) so a single search query can surface a multi-source hot topic; (3) `oracle: <query>` (**opt-in, paid**), which routes through the Oracle's `/search` endpoint and its provider fallback chain (dataforseo → gdelt → ddg). Each `oracle:` result carries its real publisher. Oracle queries are bounded to the `hotnessWindowHours` window and capped at 3 per run (`MAX_ORACLE_SOURCES_PER_RUN`); extras are ignored until the next run. No bot uses `oracle:` by default. |
