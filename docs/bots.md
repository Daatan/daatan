...
runDueBots()
  For each active bot:
    ...
    3. Fetch RSS feeds → detectHotTopics(minSources)
...
| `newsSources` | string[] | Generated by LLM | RSS feed URLs the bot monitors for hot topics. Also supports `Search: <query>` entries, which resolve to a Google News search RSS feed; each result is attributed to its real publisher (from the item's `<source>` element, falling back to the `" - Publisher"` title suffix) so a single search query can surface a multi-source hot topic. |
