...
| News article matched | 🗞️ | noisy | news-indexer integration. Reports the estimate as a **movement** (`Oracle 63% → 71%`) plus what the triggering article actually said: `stance ±x.xx · relevance x.xx · match NN%`. **stance** [-1,1] is which way it argues, **relevance** [0,1] is the Oracle's claim-aware judgment of whether it bears on the claim at all (its *square* weights the article in aggregation), **match** is the embedding cosine — listed last because it is the weakest of the three and the one proven to misrank (news-indexer#124). Each signal is omitted when unknown rather than printed as `null`. **Edited in place** (daatan#1215): a forecast's message id/chat id are persisted on `Prediction.telegramMessageId`/`telegramChatId`, and a later match edits that same message to show the latest state rather than sending a new one — falling back to a fresh send (and adopting its id) if the edit fails, e.g. outside Telegram's ~48h edit window. **Judgment-lane signals** (Signal Lanes): an optional second line — `🔎 author_lean ±x.xx (cert x.xx) · fact_signal ±x.xx · credibility x.xx · <evidence_class>` — surfaces the extractor's un-fused `author_lean`/`fact_signal`/`evidence_class`/`credibility_weight` fields when present, so a reader can see *why* an article moved the estimate, not just that it did. These are shadow-only signals (nothing in the Oracle's own aggregation reads them yet); `credibility_weight` is shown only when it differs from the neutral `1.0` default, since the credibility cutover flag is currently OFF. The line is omitted entirely when none of the four fields are present, which is most matches today. |
...
A dedicated 🔍 **rating-prompt** message, sent by `sendArticleRatingPrompt()` right after every "News article matched" push, alongside — not instead of — that notification. It exists because that main notification **edits one running message in place per prediction** (daatan#1219), so it no longer maps 1:1 to a single article/push; the rating-prompt message is always freshly sent, restoring that 1:1 mapping so a rating tap unambiguously means "this specific article's numbers."

- **1️⃣–5️⃣ button row** (1 worst, 5 best). A tap upserts one `EvidencePoolArticleFeedback` row per (article, rater) — `rating` is a plain `Int` — and refreshes the button labels with live per-value tally counts (`3️⃣ ·1`). Raters can change their mind by tapping another number.
- **A rating of 1 or 2 also opens a private DM** to the tapping rater — @DaatanClawBot must have received a `/start` from them first — with a toggle keyboard to flag which specific number was wrong: Stance / Relevance / Similarity / Probability / Author Lean / Fact Signal / Evidence Class / Credibility / Other, plus a Done button that collapses the keyboard to a plain confirmation (`Recorded: 2/5 — Stance`). 3–5 need no further detail.
- **Free-text replies** to either the rating-prompt message or the drilldown DM append to that row's `note`, once a rating already exists.
...
- Only `notifyNewsArticleMatched` gets a rating prompt in v1; no other notification type does.

Handled by the same webhook as `/rollback` (`src/app/api/telegram/rollback/route.ts`, `callback_query` branch) — Telegram supports exactly one webhook URL per bot. Authorization is by **identity** (`TELEGRAM_ADMIN_MAP`, Telegram user id → `User.id`), independent of `TELEGRAM_ROLLBACK_CHAT_IDS`'s chat-level allowlist, since the rating-prompt message lives in the noisy broadcast channel rather than the rollback admin chat.
...
| `TELEGRAM_ADMIN_MAP` | JSON map of Telegram user id → `User.id` (`{"123456":"cuid1","789012":"cuid2"}`) — identity auth for the [manual number-rating feedback](#manual-number-rating-feedback-daatan1223) buttons. Raw `process.env`, not `src/env.ts`'s validated schema — mirrors `TELEGRAM_ROLLBACK_CHAT_IDS`'s existing pattern for this same route. |
