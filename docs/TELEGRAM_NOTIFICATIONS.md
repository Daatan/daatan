...
A dedicated 🔍 **rating-prompt** message, sent by `sendArticleRatingPrompt()` right after every "News article matched" push, alongside — not instead of — that notification. It exists because that main notification **edits one running message in place per prediction** (daatan#1219), so it no longer maps 1:1 to a single article/push; the rating-prompt message is always freshly sent, restoring that 1:1 mapping so a 👍/👎 tap unambiguously means "this specific article's numbers."

- **👍 Good / 👎 Bad** buttons. A tap upserts one `EvidencePoolArticleFeedback` row per (article, rater) and refreshes the button labels with live tally counts (`👍 Good ·1`). Raters can change their mind by tapping the other button.
- **👎 also opens a private DM** to the tapping rater — @DaatanClawBot must have received a `/start` from them first — with a toggle keyboard to flag which specific number was wrong: Stance / Relevance / Similarity / Probability / Author Lean / Fact Signal / Evidence Class / Credibility / Other, plus a Done button that collapses the keyboard to a plain confirmation.
