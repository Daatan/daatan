...
| Forecast may already be resolved at creation | 🍼 | clean | `notifyBornTrueAtCreation()`, fired from `checkBornTrueAtCreation()` (`src/lib/services/bornTrueCheck.ts`), scheduled fire-and-forget from `createForecast()` right after every new forecast is persisted (daatan#1747). Re-runs the resolver's manual "AI Research" leg (`runResolutionResearch()`, extracted from `[id]/research/route.ts` — daatan#1511/#1515) immediately after creation instead of at resolve time: that leg always searches strictly before the claim's creation date as well as the deadline window, so a decisive `correct`/`wrong` verdict this soon after creation means the claim was already true or false before the forecast went live — e.g. retro#776's "Chess.com will have more than 500,000 registered users by Dec 31 2030", created while the real count was already 250M+. Covers both `NUMERIC_THRESHOLD` and free-text-threshold `BINARY` claims (the Chess.com incident was the latter) — no separate number-extraction step, the research leg's LLM already judges `claimText` as a whole. Decide-and-record: the message links to the forecast for a human to correct or delete; nothing is mutated automatically. `unresolvable`/`void` verdicts (the normal case for a genuinely open claim) never fire this. |
...
| News article matched | 🗞️ | noisy | news-indexer integration. **Fires per source article, not batched** — contrast with news-indexer's own hourly digest of new-article counts (a different repo, different mechanism: `news-indexer/src/news_indexer/worker/digest.py`, buffered via `notifier.py`'s `record_new_article()`). Composed/sent by `notifyNewsArticleMatched()` in `src/lib/services/telegram.ts`. **One fresh message per push**, laid out for skimming: the estimate as a **movement** (`Oracle 63% → 71%`) on top, followed by evidence volume (`· 3 new / 9 of 22 usable in pool` when the pool path produced the estimate — 46% of pool rows are FAILED, so the raw pool size alone overstates the evidence roughly 2× (daatan#1475); `· 3 new / 22 in pool` when composition is unknown, `· N articles` for a multi-article push on the single-run fallback); the triggering article as a link (+ a short italic **extract** — the Oracle's extracted claim when it produced one, the raw snippet otherwise); every per-article number as one bold-labelled line of a `<blockquote>` **panel** (a quote bar groups them without the "copy code" chrome clients hang on `<pre>` blocks); then the forecast as a link named by its claim text, with the 1️⃣–5️⃣ rating buttons attached (see below). Panel rows, in two groups (daatan#1661). **Live** — read by the estimate: **stance** [-1,1] — which way the article argues (signed, `(cert x.xx)` = the extractor's certainty in that reading); **relevance** [0,1] — the Oracle's claim-aware judgment (its *square* weights the article in aggregation); **range** — the confidence band, omitted when under 2 points wide. Then an `<i>not in estimate:</i>` marker (`SHADOW_MARKER`, present only when at least one shadow row rendered) and the **shadow** rows — captured and shown, read by nothing in the Oracle's aggregation yet: the Signal Lanes fields `author_lean (cert)` / `fact_signal` / `credibility` / `class` (`credibility` only when it differs from the neutral `1.0` default, since the credibility cutover flag is OFF) and the elicited fields `consensus` (article-level `consensus_view`: expects_yes / expects_no / divided, retro#686), `report_kind` (level / change, retro#686), `reader` (retro#681 — the extractor's confidence in its own READING, as against the `cert` on `stance`, which is the SOURCE's; the trap is appended in brackets when set, `low (negation)`, and that half is the informative one) and `quantity` (retro#683 — the figure the claim reports and the relation the ARTICLE asserts, `214 daily departures` / `< 40 million tonnes`, never a verdict on the question's own threshold, which is decided in code). The last three are taken from `claims_detail[0]` — the same claim the extract line quotes, deliberately per-claim rather than the article-level rollup, so every number in the panel describes one claim. `quantity` is rendered **verbatim and never normalised**: the extractor legitimately returns the same figure as `452 / "thousand active US Army personnel"` and `452000 / "active US Army personnel"`, so folding either way would print a number the article never wrote. The live/shadow split is two arrays in `notifyNewsArticleMatched` (`liveRows` / `shadowRows`); when Oracle 1.5 graduates a field, move its row and update `/help/rating-numbers` + this row — nothing else knows which is which. The embedding cosine (**match**) is **no longer a row** since daatan#1661: it is why news-indexer pushed, not evidence about the claim, and under Funnel v2 the judge's `relevance` is the gate; it stays persisted on `ArticleRatingPrompt.snapshotSimilarity`, and the drilldown DM no longer offers `Similarity` (the `SIMILARITY` enum value remains for existing rows). **retro#763's `grounds` gets a row (`grounds`), decided 2026-08-30** — rendered as the spelled-out kind and the basis phrase (`official statement · the ministry's 12 March statement`), per-claim from the same claim the panel quotes; it is the one shadow field that is readable by construction, being the answer to *why does this article think so*, and the basis phrase is what lets a rater see two articles repeating one statement. **retro#697's `claim_actor` / `claim_predicate` / `claim_scope` get no row either, decided 2026-08-30** — the rule below is *decide and record*, not *always add*, and the answer here is no on two counts: they decompose the **question's** event rather than this article, so the same three values repeat identically on every message for a given forecast and discriminate between none of them; and what they say (`Likud` · `wins the most seats` · `Israel, by the deadline`) is a restatement of the claim text the forecast link is already named by, two lines further down. Revisit if Oracle 1.5 Phase 4 ever makes them disagree across articles — a decomposition that *differs* between two articles on one question is a real signal and would earn a row. Every row is omitted when unknown rather than printed as `null`; the whole `<blockquote>` is omitted when no row rendered. Freshly sent, not edited in place: an edit never resurfaces in the channel feed, and a rating tap must map 1:1 to exactly the numbers shown. (The daatan#1215/#1219 edit-in-place mechanics were retired by this redesign; `Prediction.telegramMessageId`/`telegramChatId` are no longer written.) |
| High AI confidence (≥80%) | 📈 | clean | `src/lib/services/context.ts` — fires when the AI estimate **crosses** 80 from below (any confidence-writing path: news-indexer push, user "analyze context", admin backfill). Crossing-based, so a forecast hovering at 82 doesn't re-alert. Adds a "settled" line when the Oracul reports the outcome as an accomplished fact (resolution candidate). |
...
|---|---|---|---|---|
| Oracul forecast unavailable | 🚨 | clean | 5 min (global) | `GET /api/cron/oracle-health` |
...
| Oracul search unavailable | ⚠️ | noisy | 5 min (global) | `src/lib/services/oracleSearch.ts` |
| Market ⇄ Oracul divergence | 📊 | noisy | single-shot per crossing; hysteresis re-arms only once the gap falls to ≤15pt (not merely <20pt), and an atomic claim (`updateMany` gated on the alert column being null) prevents double-fires across overlapping cron runs | `GET /api/cron/external-market-sync` (hourly) → `checkMarketDivergence()` in `src/lib/services/external-markets.ts` — fires when a linked Polymarket/Kalshi market's implied probability and our Oracul estimate differ by more than 20pt |
