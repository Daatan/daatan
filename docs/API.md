...
List the forecast's evidence pool (`evidence_pool_articles`) — every article `analyze`/`news-indexer`/`backfill` have extracted a signal from. See `docs/DATABASE.md` "Evidence pool"; since v1.60.0 the pool aggregate *is* the persisted estimate on all three paths. Response also carries `poolSize`/`usableSize` (daatan#1521) — current-version row counts (superseded corrections excluded), `usableSize` filtered through the same `isUsablePoolRow` predicate the aggregate itself reads. Raw `articles.length` overstates usable evidence by roughly 2x, since it includes `FAILED`/`PENDING` rows the aggregate never sees.
...
Re-extract the named forecasts' strongest evidence rows against the *current* extractor and re-publish their estimates — the backward half of the estimation-quality plan's Track A ([daatan#1493](https://github.com/Daatan/daatan/issues/1493)), for readings taken before the retro#549 fact/stance sign-error guard and the extractor prompt fix landed. **Body** `{ ids: string[], mode?: 'dry-run' | 'apply', scope?: 'a6' | 'usable' | 'amnesty' }`. **`scope` defaults to `a6`**: current-version *usable* pool rows with `|stance| >= 0.7` **and** `certainty >= 0.7` (search-redirect URLs excluded — there is no article behind them to re-read), batched at 15 rows per Oracul analysis. `usable` widens the target to the whole currently-usable pool (retro#626 blanket recompute). `amnesty` is the one-time pre-08-05 `oracle_null_final` re-drive ([daatan#1547](https://github.com/Daatan/daatan/issues/1547)) — rows finalized under the pre-#1253 12s-timeout regime, scoped to current-version rows on `ACTIVE` forecasts with a future `resolveByDatetime`. **`mode` defaults to `dry-run`**: it reports the target set, usable pool size and published confidence, and writes nothing. Max 10 ids per call.
...
```json
{
  ...
  "version": "1.22.6",    // NEXT_PUBLIC_APP_VERSION, baked from package.json at build time
