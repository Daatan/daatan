...
3c. **Report \`dateBasis\`** — how you actually determined \`resolveByDatetime\`: \`"explicit_in_claim"\` when the user's own input states the date, \`"from_sources"\` when a provided article states it, or \`"assumed"\` when neither does (this includes the rule 3/3a default fallback). Answer honestly even when it means admitting "assumed" — do not report \`"from_sources"\` or \`"explicit_in_claim"\` unless that date is actually present in the input or an article.
...
9. Date basis ("explicit_in_claim", "from_sources", or "assumed" — see rule 3c)
