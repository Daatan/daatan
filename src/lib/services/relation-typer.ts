 ...
 *
 * Pairs typed "independent" are not recorded — there is no row kind for "we
 * looked and found nothing" — so they are re-asked on a later run. At ~110
 * candidate pairs site-wide that is cents per day; if the candidate set grows
 * into thousands, add a typed-pair ledger before raising the limit.
 */
...
const KIND_MAP: Record<Exclude<TyperVerdict['relation'], 'independent'>, QuestionRelationKind> = {
  alias: 'ALIAS',
...
export function toProposal(pair: CandidatePair, v: TyperVerdict): RelationProposal | null {
  if (v.relation === 'independent' || v.confidence < MIN_CONFIDENCE) return null
...
export async function runRelationTyper(opts: { limit?: number; dryRun?: boolean } = {}): Promise<TyperRunSummary> {
  ...
  for (const pair of pairs) {
    ...
    if (!verdict) { summary.failed++; continue }
    summary.typed++
    if (verdict.relation === 'independent') { summary.independent++; continue }
    const proposal = toProposal(pair, verdict)
    if (!proposal) { summary.lowConfidence++; continue }
    if (dryRun) {
      summary.proposals!.push({ ...proposal, aClaim: pair.aClaim, bClaim: pair.bClaim })
      continue
    }
