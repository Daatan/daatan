/**
 * Oracle-backed news source for bots. An `oracle: <query>` entry in a bot's
 * newsSources runs the query through the Oracle's /search endpoint, which shares
 ...
 *
 * This is a paid path: each query is one Oracle search. The runner caps how many
...
/**
 * Runs each query through the Oracle's /search endpoint and maps the results to
 * RssItems. Results are bounded to the hot-topic window via `dateFrom` so the
 * Oracle only returns articles recent enough to form a hot topic. Failures on
 * individual queries are skipped (oracleSearch never throws and returns null
 * when the Oracle is unconfigured or empty).
 */
